...
A dashboard built with [Dashfy](https://github.com/dashfy/dashfy), scaffolded from the `vite-starter` template (a minimal starting point).
...
Pass any secrets your extensions need at runtime with `-e` — the image never contains
them. To change dashboards without rebuilding, mount your config; the server watches it
...
This installs the extension's npm package and sets it up in `src/App.tsx`,
