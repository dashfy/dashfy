...
describe('searchRegistries', () => {
  it('returns all items for a registry with no query', async () => {
    const { results } = await searchRegistries(['@dashfy'], { registries })
    expect(results).toHaveLength(1)
    expect(results[0]?.registry).toBe('@dashfy')
    expect(results[0]?.items.length ?? 0).toBeGreaterThan(0)
  })

  it('filters items by query', async () => {
    const { results } = await searchRegistries(['@dashfy'], { registries, query: 'github' })
  ...
  it('applies limit and offset', async () => {
    const all = await searchRegistries(['@dashfy'], { registries })
    const total = all.results[0]?.items.length ?? 0
    expect(total).toBeGreaterThan(1)

    const limited = await searchRegistries(['@dashfy'], { registries, limit: 1 })
    expect(limited.results[0]?.items).toHaveLength(1)

    const offset = await searchRegistries(['@dashfy'], { registries, offset: 1 })
  ...
  it('collects per-registry errors when continueOnError is set', async () => {
    const { results, errors } = await searchRegistries(['@dashfy', '@missing'], {
      registries,
      continueOnError: true,
    })
    expect(results.some((group) => group.registry === '@dashfy')).toBe(true)
